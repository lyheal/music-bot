...
const { canModifyQueue, STAY_TIME, LOCALE } = require("./LayerCoding");



module.exports = {
  async play(song, message) {
    const { SOUNDCLOUD_CLIENT_ID } = require("./LayerCoding");
    ...
    try {
      ...
      return message.channel.send(
        `"**Error: ${error.message}**"`, { error: error.message ? error.message : error })
      ;
    }
    ...
    collector.on("collect", (reaction, user) => {
      ...
      switch (reaction.emoji.name) {
        ...
        case "🔁":
          ...
          queue.textChannel
            .send(
              `**reply is now ${queue.loop ? '**on**' : '**off**'}**`)
